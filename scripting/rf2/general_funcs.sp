...
void GameOver()
{
	...
	EmitSoundToAllEx(SND_GAME_OVER);
	...
	bool updatePos=true, float soundTime=0.0)
{
	...
	if (times == 1)
	{
		EmitSoundToClientEx(client, sound, entity, channel, level, flags, volume, pitch, speakerentity, origin, dir, updatePos, soundTime);
		return;
	}
	...
	for (int i = 1; i <= times; i++)
	{
		...
		EmitSoundToClientEx(client, sound, entity, channel, level, flags, vol, pitch, speakerentity, origin, dir, updatePos, soundTime);
	}
}
